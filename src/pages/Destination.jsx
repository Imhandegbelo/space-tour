...
export default function Destination() {
  ...
  return (
    <div className="h-screen w-screen lg:pt-10 bg-destination-mobile md:bg-destination-tablet lg:bg-destination-desktop bg-cover overflow-y-clip">
      <Navbar />
      <div className="flex flex-col gap-16 lg:px-36 lg:mt-36 items-center justify-between text-gray-300">
        <h1 className="w-full lg:pb-10 text-white text-base md:text-[1.75rem] font-Barlow+Condensed mt-10 pl-8 lg:my-0 lg:ml-0 lg:mct-36 lg:mb-24">
          <span className="opacity-30 mr-3">01</span>PICK YOUR DESTINATION
        </h1>

        <div className="flex flex-col justify-between lg:flex-row h-4/5 w-full px-6 md:px-20">
          <div className="flex w-full px-8 md:px-0 pb-16 lg:pb-0">
            <img
              src={images[active]}
              alt={"name"}
              className="shrink animate-[spin_60s_linear_infinite] mx-auto"
            />
          </div>
          <div className="grid gap-1 lg:gap-16 basis-1/2 lg:mt-10 text-center lg:text-left">
            <div className="flex justify-between w-full md:w-3/5 mx-auto lg:w-full">
              {data.destinations.map((dest,index) => (
                <button
                  key={dest.name}
                  onClick={()=>setActive(index)}
                  className={`border-b-4 pb-3 uppercase ${active === index? "border-white": "border-white/50"} hover:border-white tracking-[2.7px]`}
                >
                  {dest.name}
                </button>
              ))}
            ...
            <div className="border-b border-white/30 py-10 mb-10 lg:mb-0 w-full">
              <h3 className="text-white font-Bellefair text-[3.5rem] md:text-[5rem] lg:text-[6.25rem] mb-6 uppercase leading-[3rem]">
